package com.abelyaev.balance.service;

import com.abelyaev.balance.constants.Constant;
import com.abelyaev.balance.model.dto.BalanceInfo;
import com.abelyaev.balance.model.dto.TableInfoDto;
import com.abelyaev.balance.model.entity.FilesEntity;
import com.abelyaev.balance.producer.BalanceProducer;
import com.abelyaev.balance.repository.FilesRepository;
import com.abelyaev.balance.utils.ParseFileUtils;
...
public class FileParserService {
    ...
    private int findSaldoIndex(XSSFSheet sheet) {
        int saldoIndex = IntStream.range(0, 10)
                .filter(i -> Constant.BALANCE_END_PERIOD.equals(parseFileUtils.setCellString(parseFileUtils
    ...
    private TreeSet<String> getCompanyDuplicates(XSSFSheet sheet, TableInfoDto tableInfoDto) {
        ...
        for (int i = 6; i <= sheet.getLastRowNum(); i++) {
            ...
            if (Constant.FIRST_TABLE_BEGIN_SIX_ZERO.equals(stringCell) || Constant.FIRST_TABLE_BEGIN_SIX_TWO.equals(stringCell)) {
                isFirstTable = Boolean.TRUE;
            }
            if (Constant.SECOND_TABLE_BEGIN_SIX_ZERO.equals(stringCell) || Constant.SECOND_TABLE_BEGIN_SIX_TWO.equals(stringCell)) {
